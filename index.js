...
document.addEventListener('DOMContentLoaded', () =>{
    ...
    let moveCount = 1; // Initialize the new move count
    ...
    // Event listener or the "Play Again" button
    ...
    document.querySelector('.set-pos').addEventListener('click', () =>{
        const fen = prompt("Enter the FEN notation for the desired notation!");
        if (fen !== null) {
            if (game.load(fen)) {
                board.position(fen);
                moveHistory.textContent = '';
    ...
    document.querySelector('.flip-board').addEventListener('click', () =>{
        board.flip();
        makeRandomMove();
        // Toggle user's color after lipping the board
